...
class CASMeshDataset(Dataset):
    def __init__(
        ...
        self.case_ids = sorted(path.stem for path in mesh_dir.glob("CAS_*.obj"))
        if limit > 0:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing CAS CTA images, e.g. CAS_XXXX_image.nii.gz.",
    )
    parser.add_argument(
        "--mesh-dir",
        type=Path,
    ...
    dataset = CASMeshDataset(args.image_dir, args.mesh_dir, limit=args.limit)
